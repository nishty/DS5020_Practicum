.. 31b8e172-b470-440e-83d8-e6b185028602::31b8e172-b470-440e-83d8-e6b185028602
...
From the charts and stat, we can observe that the county with the highest admission is Madison, the lowest is King.

```{r}
perc_admission_per_county = admissions_data %>%
  select(county_code, admissions) %>%
  group_by(county_code) %>%
  summarise(perc = round(n() / sum(admissions) * 100, 3)) %>%
  arrange(desc(perc))

perc_admission_per_county
# get basic stat of admission
summary(perc_admission_per_county)
# check the min admission county
perc_admission_per_county %>%
  slice(which.min(perc))
perc_admission_per_county %>%
...
perc_admission_per_county %>%
    top_n(10) %>%
    mutate(county_code = factor(county_code, levels = county_code)) %>%
    ggplot(aes(x=county_code, y=perc)) +geom_col() + labs(title='Top 10 County /w Most Admissions')
